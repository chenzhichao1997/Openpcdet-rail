...
class VoxelNeXtHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        super().__init__()
        self.model_cfg = model_cfg #传入的是head config
        self.num_class = num_class#4
        self.grid_size = grid_size#1600,928,40
        self.point_cloud_range = torch.Tensor(point_cloud_range).cuda()
        self.voxel_size = torch.Tensor(voxel_size).cuda()#0.4,0.4,0.3
        self.feature_map_stride = self.model_cfg.TARGET_ASSIGNER_CONFIG.get('FEATURE_MAP_STRIDE', None)#stride 8
        ...
        self.double_flip = self.model_cfg.get('DOUBLE_FLIP', False)   #False
    ...
    def assign_targets(self, gt_boxes, num_voxels, spatial_indices, spatial_shape):
        ...
        Returns:
        """
        target_assigner_cfg = self.model_cfg.TARGET_ASSIGNER_CONFIG#真值的配置文件.
        #gt_boxes [4,23,8]     x,y,z,dx,dy,dz,rot,classes
        batch_size = gt_boxes.shape[0]#4
        ret_dict = {
            'heatmaps': [],
            'target_boxes': [],
            'inds': [],
        ...
        all_names = np.array(['bg', *self.class_names]) #all_name ['bg' 'road_vehicle' 'person' 'buffer_stop' 'train']

        for idx, cur_class_names in enumerate(self.class_names_each_head):
            heatmap_list, target_boxes_list, inds_list, masks_list, gt_boxes_list = [], [], [], [], []
            for bs_idx in range(batch_size):
                cur_gt_boxes = gt_boxes[bs_idx]
                gt_class_names = all_names[cur_gt_boxes[:, -1].cpu().long().numpy()]#gt的所有类.
                ...
                # gt_boxes_sigle_head [23,8] Tensor type.
            ...
            ret_dict['heatmaps'].append(torch.cat(heatmap_list, dim=1).permute(1, 0))#torch.Size([6007, 4])
            ret_dict['target_boxes'].append(torch.stack(target_boxes_list, dim=0))#torch.Size([8, 500, 4])
            ret_dict['inds'].append(torch.stack(inds_list, dim=0))#torch.Size([4, 500])
            ret_dict['masks'].append(torch.stack(masks_list, dim=0))#torch.Size([4, 500])
            ret_dict['gt_boxes'].append(gt_boxes_list)#list:4  torch.Size([9, 7])
    ...
    #     #上面的步骤对x,y进行了转换，转换到了相对坐标
    #     center = torch.cat((coord_x[:, None], coord_y[:, None]), dim=-1)#x,y 坐标.
    #     center_int = center.int()
    #     center_int_float = center_int.float()
    #
    #     dx, dy, dz = gt_boxes[:, 3], gt_boxes[:, 4], gt_boxes[:, 5]
    #     dx = dx / self.voxel_size[0] / feature_map_stride
    #     dy = dy / self.voxel_size[1] / feature_map_stride   #都转换到了体素空间坐标系，按步长.
    #
    ...
    #         # check the box
    #         if dx[k] <= 0 or dy[k] <= 0:
    #             print("dx[k] <= 0 or dy[k]<= 0")
    #             continue
    #
    #         if not (0 <= center_int[k][0] <= spatial_shape[1] and 0 <= center_int[k][1] <= spatial_shape[0]):
    #             print("0 <= center_int[k][0] <= spatial_shape[1] and 0 <= center_int[k][1] <= spatial_shape[0]")
    #             continue
    #
    #         cur_class_id = (gt_boxes[k, -1] - 1).long()
    #         distance = self.distance(spatial_indices, center[k]) #计算预测框与每个真实框中心的距离.
    #         inds[k] = distance.argmin()#预测框与真实值进行匹配的真值.
    #         mask[k] = 1
    #
    ...
    #
    #         ret_boxes[k, 0:2] = center[k] - spatial_indices[inds[k]][:2]#计算真实框距离对应体素中心点的位置x,y。
    #         ret_boxes[k, 2] = z[k]
    ...
    #
    #     return heatmap, ret_boxes, inds, mask    #heatmap中心点的类别图， ret_boxes是真值对应于体素的相对坐标x,y, inds
    def assign_target_of_single_head(
        ...
        """
        为单一检测头分配监督目标，生成类别 heatmap 和回归框。
        支持按类别配置不同的高斯参数（overlap 和 min_radius）。

        Args:
            num_classes: 类别数
            gt_boxes: (N, 8) [x, y, z, dx, dy, dz, heading, cls]
            num_voxels: 所有有效体素数量（决定 heatmap 大小）
            spatial_indices: 所有有效体素的坐标（N, 3）
            spatial_shape: BEV 特征图大小 [y, x]
            feature_map_stride: voxel 坐标到特征图的缩放比例
            num_max_objs: 最大 GT 数目

        Returns:
            heatmap: (num_classes, num_voxels)
            ret_boxes: 编码后的 GT 框信息
            inds: 每个 GT 匹配的 voxel 索引
            mask: 有效目标掩码
        """
        # ["road_vehicle","person","buffer_stop","train","crowd"]
        self.class_gaussian_overlap = {
            0: 0.1,
            1: 0.3,
            2: 0.2,
            3: 0.15,
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            # === 按类别动态设置高斯参数 ===
            this_overlap = self.class_gaussian_overlap.get(cur_class_id.item(), 0.1)
            this_min_radius = self.class_min_radius.get(cur_class_id.item(), 2)

            this_radius = centernet_utils.gaussian_radius(dx[k], dy[k], min_overlap=this_overlap)
            this_radius = max(this_min_radius, int(this_radius))
            radius[k] = this_radius
            ...
            if 'gt_center' in self.gaussian_type:
                centernet_utils.draw_gaussian_to_heatmap_voxels(
                    heatmap[cur_class_id], distance, this_radius * self.gaussian_ratio
                )

            if 'nearst' in self.gaussian_type:
                centernet_utils.draw_gaussian_to_heatmap_voxels(
                    heatmap[cur_class_id],
                    self.distance(spatial_indices, spatial_indices[inds[k]]),
                    this_radius * self.gaussian_ratio
                )
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            hm_loss *= self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['cls_weight']#1.热图分类损失
            ...
            )#2.这部分的masks就是匹配的框.  masks [1,1,1,1,.....] length 500
            ...
            if self.iou_branch:
                ...
                #print("hmloss:",hm_loss.item(),"loc_loss:",loc_loss.item(),"iou_loss:",iou_loss.item(),"iou_reg_loss:",iou_reg_loss.item())
    ...
    def forward(self, data_dict):
        ...
        self.forward_ret_dict['batch_index'] = batch_index # batch index
        # 每个头部的输出，即预测结果
        pred_dicts = []
