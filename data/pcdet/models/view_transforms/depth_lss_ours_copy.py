...
from pcdet_fanet.ops.bev_pool import bev_pool
...
class DepthLSSTransform_ours(nn.Module):
    """
    ...
    def __init__(self, model_cfg):
        ...
        # dx每个维度上的网格间距;(0.6,0.6,20)
        # bx每个维度上的起点的中心点(0.3,-53.7,0)
        # nx每个维度上的网格数量(430,180,1)
        self.dx = nn.Parameter(dx, requires_grad=False)
        self.bx = nn.Parameter(bx, requires_grad=False)
        self.nxx = nn.Parameter(nx, requires_grad=False)

        self.C = out_channel
        self.frustum = self.create_frustum() #创建了一个视锥空间 [dS,fH,fW,(x,y,z)]
    ...
    def create_frustum(self):
        ...
        frustum = torch.stack((xs, ys, ds), -1)#创建出frustum的坐标空间.
    ...
    def bev_pool(self, geom_feats, x):
        ...
        kept = (
            (geom_feats[:, 0] >= 0)
            & (geom_feats[:, 0] < self.nxx[0])
            & (geom_feats[:, 1] >= 0)
            & (geom_feats[:, 1] < self.nxx[1])
            & (geom_feats[:, 2] >= 0)
            & (geom_feats[:, 2] < self.nxx[2])
        )
        x = x[kept]
        geom_feats = geom_feats[kept]
        x = bev_pool(x, geom_feats, B, self.nxx[2], self.nxx[0], self.nxx[1])
    ...
    def forward(self, batch_dict):
        ...
        camera_intrinsics = batch_dict['camera_intrinsics']  # [4,1,4,4]  相机
        ...
        for b in range(batch_size):
            ...
            # get 2d coords
            dist = cur_coords[:, 2, :]    #目前转换为了图像坐标系，最后1维是深度.
            cur_coords[:, 2, :] = torch.clamp(cur_coords[:, 2, :], 1e-5, 1e5)
            cur_coords[:, :2, :] /= cur_coords[:, 2:3, :]# 得到了激光雷达在图像中的2d坐标. x/z, y/z
            ...
            cur_coords = cur_coords[..., [1, 0]]#切换为hw,也就是网格坐标
