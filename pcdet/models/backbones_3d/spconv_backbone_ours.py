...
class Voxel2Backbone_ours(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        if self.use_attentation:
            x_conv1 = self.attentation(self.conv1(input_sp_tensor))
            x_conv2 = self.attentation(self.conv2(x_conv1))
            x_conv3 = self.attentation(self.conv3(x_conv2))
            x_conv4 = self.attentation(self.conv4(x_conv3))
            x_conv5 = self.attentation(self.conv5(x_conv4))
            x_conv6 = self.attentation(self.conv6(x_conv5))
        else:
        ...
        if self.model_cfg.FUSION == "None":
            ...
            x_conv4.indices = torch.cat(
                [x_conv4.indices, x_conv5.indices])
            x_conv4 = self.bev_out(x_conv4)
