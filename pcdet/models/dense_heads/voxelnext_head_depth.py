...
class VoxelNeXtHead_depth(nn.Module):
    ...
    def get_loss(self):
        ...
        cls_loss_src = self.cls_loss_func(self.forward_ret_dict['d_pre'], self.forward_ret_dict['d_lable'])  # [N, M]
        cls_loss = cls_loss_src.sum() / self.forward_ret_dict['batch_size']

        cls_loss = cls_loss * 1

        tb_dict['rpn_loss'] = loss.item() + cls_loss.item()
        loss = loss + cls_loss
        # print(loss.item())
        cls_loss.item()
        return loss, tb_dict
    ...
    def forward(self, data_dict):
        ...
        self.forward_ret_dict['d_lable'] = data_dict['d_lable']
        self.forward_ret_dict['d_pre'] = data_dict['d_pre']
