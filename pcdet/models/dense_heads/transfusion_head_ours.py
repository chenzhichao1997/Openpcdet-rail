...
from ..model_utils_copy.transfusion_utils import clip_sigmoid
from ..model_utils_copy.basic_block_2d import BasicBlock2D
from ..model_utils_copy.transfusion_utils import PositionEmbeddingLearned, TransformerDecoderLayer
...
class TransFusionHead_ours(nn.Module):
    """
    ...
    def __init__(
        ...
        super(TransFusionHead_ours, self).__init__()
    ...
    def predict(self, inputs):
        ...
        lidar_feat = self.shared_conv(inputs)#进行特征降维 [2,128,180,180]

        lidar_feat_flatten = lidar_feat.view(
            batch_size, lidar_feat.shape[1], -1
        )#b,c,h*w
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(lidar_feat.device) #bev的中心点坐标 【2,32400,2】
    ...
    def forward(self, batch_dict):
        feats = batch_dict['spatial_features'] #获取待检测的特征,stride = 8
        res = self.predict(feats)#获得检测框,200个.
