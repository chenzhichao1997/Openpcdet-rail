...
def decode_bbox_from_heatmap(heatmap, rot_cos, rot_sin, center, center_z, dim,
    ...
    if score_thresh is not None:
        ...
        #mask = apply_score_threshold(final_scores, final_labels, score_thresh)
    ret_pred_dicts = []
...
def decode_bbox_from_voxels_nuscenes(batch_size, indices, obj, rot_cos, rot_sin,
    ...
    #         # 将其转换为 tensor
    #         score_thresh = torch.tensor(score_thresh, device=final_scores.device).float()
    #     elif isinstance(score_thresh, float):
    #         score_thresh = torch.tensor([score_thresh] * (final_class_ids.max().item() + 1),
    #                                     device=final_scores.device).float()
    #
    #     # 获取每个预测类别对应的阈值
    #     cls_thresholds = score_thresh[final_class_ids]
    #     mask &= (final_scores > cls_thresholds)
    ret_pred_dicts = []
    for k in range(batch_size):
        cur_mask = mask[k]
        cur_boxes = final_box_preds[k, cur_mask]
